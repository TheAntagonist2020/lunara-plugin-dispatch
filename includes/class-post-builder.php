...
class Lunara_Dispatch_Post_Builder {
	...
	public function split_into_individual_posts( $html, array $section_image_map, $post_type, $post_status = 'draft', array $run_context = array() ) {
		...
		foreach ( $sections as $section_index => $section ) {
			...
			$source_packet_mode = ! empty( $run_context['source_packet_mode'] );
			$quality_failure = $source_packet_mode
				? $this->source_packet_section_failure( $title, $body )
				: $this->publishable_section_failure( $title, $body );
			if ( '' !== $quality_failure ) {
				$this->last_quality_gate_skips[] = array(
			...
			if ( $source_packet_mode ) {
				update_post_meta( $new_id, '_lunara_dispatch_source_packet', '1' );
				update_post_meta( $new_id, '_lunara_dispatch_ai_error_code', sanitize_key( (string) ( $run_context['ai_error_code'] ?? '' ) ) );
			}
	...
	/**
	 * Validate the deliberately non-publishable, deterministic fallback shape.
	 * This is a narrow bypass for packets generated by the internal builder only;
	 * it does not weaken the editorial gate for AI copy.
	 */
	private function source_packet_section_failure( $title, $body ) {
		if ( '' === trim( (string) $title ) || '' === trim( (string) $body ) ) {
			return 'source packet missing title or body';
		}
		if ( false === strpos( (string) $body, Lunara_Dispatch_Source_Packet_Builder::MARKER ) ) {
			return 'source packet marker missing';
		}
		$paragraphs = substr_count( strtolower( (string) $body ), '<p' );
		if ( $paragraphs < 3 || $paragraphs > 6 ) {
			return 'source packet must contain three to six paragraphs';
		}
		if ( false === stripos( (string) $body, 'Original report:' ) || false === stripos( (string) $body, '<a ' ) ) {
			return 'source packet provenance link missing';
		}
